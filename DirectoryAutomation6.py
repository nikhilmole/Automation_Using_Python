...
def DirectoryWatcher(DirName):
    ...
    flag = os.path.isabs(DirName)       #This method is used to path is absolute or not.

    if (flag == False):
        DirName = os.path.abspath(DirName)      #This method is used to convert relative path into absolute path.
    ...
    if(exist == True):
        for foldername, subfoldername, filename in os.walk(DirName):
            for name in filename:
                print("File path is : ",os.path.join(foldername,name))          #join method is used to Display full path.
